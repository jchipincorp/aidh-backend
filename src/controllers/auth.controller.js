...
async function verifyToken(req, res) {
  const result = await db.query(
    `SELECT u.id, u.email, u.role, u.email_verified_at,
            sp.plan, sp.trial_ends_at
       FROM users u
       LEFT JOIN subscriber_profiles sp ON sp.user_id = u.id
      WHERE u.id = $1 AND u.deleted_at IS NULL`,
    [req.user.sub]
  );
  if (!result.rows.length) return res.status(401).json({ valid: false });
  const user = result.rows[0];
  return res.json({
    valid: true,
    user: {
...
module.exports = { register, signin, verifyEmail, verifyToken, issueToken, BCRYPT_ROUNDS };
