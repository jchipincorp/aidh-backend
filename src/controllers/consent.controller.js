...
async function grantConsent(req, res) {
  ...
  // IDOR fix: the match must belong to the calling subscriber. Previously
  // this only checked that the match existed at all -- any authenticated
  // subscriber could grant consent on ANY other subscriber's match and
  // receive back a fully valid consent token for it, which a practitioner
  // could then use to pull that OTHER subscriber's real health snapshot.
  // Live-exploited end-to-end against this exact code before this fix
  // (register as unrelated "attacker", POST practitionerMatchId belonging
  // to a "victim", receive token, use it via getDataForPractitioner to
  // read the victim's real slider snapshot) -- confirmed fixed after.
  const owner = await db.query(
    `SELECT pm.id, pm.discipline_id FROM practitioner_matches pm
...
async function revokeConsent(req, res) {
  ...
  // Same IDOR fix as grantConsent above: only the owning subscriber may
  // revoke consent on their own match. Previously any authenticated
  // subscriber could revoke ANY other subscriber's active consent by
  // guessing/obtaining their match id -- a denial-of-service on someone
  // else's practitioner relationship. Live-exploited before this fix,
